...
class AvisotaBackendOutbox extends BackendModule
{
	...
	protected function compile()
	{
		...
		$this->loadLanguageFile('orm_avisota_mailing_outbox');
		$this->loadLanguageFile('orm_avisota_mailing');
	...
	protected function details()
	{
		...
		$recipient = $this->Database
			->prepare("SELECT * FROM orm_avisota_mailing_outbox_recipient WHERE pid=? $where ORDER BY email")
	...
	{
		$this->Database
			->prepare("DELETE FROM orm_avisota_mailing_outbox WHERE id=?")
			->execute($this->Input->get('id'));
		$this->Database
			->prepare("DELETE FROM orm_avisota_mailing_outbox_recipient WHERE pid=?")
			->execute($this->Input->get('id'));

		$_SESSION['TL_CONFIRM'][] = $GLOBALS['TL_LANG']['orm_avisota_mailing_outbox']['removed'];
	...
	protected function outboxes()
	{
		...
		$outbox = $this->Database->execute(
			"
				SELECT
					...
					o.tstamp,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE r.pid=o.id) as recipients,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE r.pid=o.id AND r.send=0) as outstanding,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE r.pid=o.id AND r.failed='1') as failed
				FROM
					orm_avisota_mailing_outbox o
				INNER JOIN
		...
		while ($outbox->next()) {
			...
			$resultSet = $this->Database
				->prepare(
				'SELECT source, sourceID, COUNT(id) as recipients FROM orm_avisota_mailing_outbox_recipient WHERE pid=? GROUP BY source'
			)
				->execute($outbox->id);
		...
		switch ($recipient->source) {
			case 'list':
				...
				if ($list->next()) {
					...
					$source['title']       = sprintf(
						'%s: %s',
						$GLOBALS['TL_LANG']['orm_avisota_mailing_outbox']['recipient_list'],
						$list->title
					);
			...
			case 'mgroup':
				...
				if ($memberGroup->next()) {
					...
					$source['title']       = sprintf(
						'%s: %s',
						$GLOBALS['TL_LANG']['orm_avisota_mailing_outbox']['mgroup'],
						$memberGroup->name
					);
					$source['linkedTitle'] = sprintf(
	...
	protected function getOutbox()
	{
		...
		$outbox = $this->Database
			...
			"SELECT
					*,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE o.id=r.pid) as recipients,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE o.id=r.pid AND r.send=0) as outstanding,
					(SELECT COUNT(id) FROM orm_avisota_mailing_outbox_recipient r WHERE o.id=r.pid AND r.failed='1') as failed
				FROM
					orm_avisota_mailing_outbox o
				WHERE
					id=?"
