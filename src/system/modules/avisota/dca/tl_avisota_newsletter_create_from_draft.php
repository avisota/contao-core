...
class orm_avisota_newsletter_create_from_draft extends Backend
{
	/**
	...
	public function onsubmit_callback(DataContainer $dc)
	{
		...
		$newsletterDraft = $this->Database
			->prepare("SELECT * FROM orm_avisota_newsletter_draft WHERE id=?")
			->execute($draftId);
		if ($newsletterDraft->next()) {
			...
			$newsletter = $this->Database
				->prepare(
				"INSERT INTO orm_avisota_newsletter (" . implode(",", array_keys($newsletterDraftData)) . ") VALUES ($value)"
			)
				->execute($newsletterDraftData);
			$newsletterId         = $newsletter->insertId;

			$content = $this->Database
				->prepare("SELECT * FROM orm_avisota_newsletter_draft_content WHERE pid=?")
				->execute($draftId);
			...
			while ($content->next()) {
				...
				$newsletter = $this->Database
					->prepare(
					"INSERT INTO orm_avisota_newsletter_content (" . implode(
						",",
			...
			$_SESSION['TL_INFO'][] = $GLOBALS['TL_LANG']['orm_avisota_newsletter_create_from_draft']['created'];
			$this->redirect('contao/main.php?do=avisota_newsletter&table=orm_avisota_newsletter_content&id=' . $newsletterId);
		}
	}
}
