...
class orm_avisota_newsletter extends Backend
{
	/**
	...
	public function updatePalette()
	{
		if ($this->Input->get('act') == 'edit') {
			$category = $this->Database
				->prepare(
				'SELECT c.*
						   FROM orm_avisota_newsletter_category c
						   INNER JOIN orm_avisota_newsletter n
						   ON c.id=n.pid
						   WHERE n.id=?'
			)
	...
	public function editNewsletter($row, $href, $label, $title, $icon, $attributes)
	{
		return (!$row['sendOn'] && ($this->User->isAdmin || count(
			preg_grep('/^orm_avisota_newsletter::/', $this->User->alexf)
