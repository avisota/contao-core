...
class orm_avisota_newsletter_content extends Backend
{
	...
	/**
	 * Check permissions to edit table orm_avisota_newsletter_content
	 */
		...
		switch ($this->Input->get('act')) {
			...
			case 'copyAll':
				...
				$contentElement = $this->Database
					->prepare("SELECT id FROM orm_avisota_newsletter_content WHERE pid=?")
					->execute(CURRENT_ID);
	...
	protected function checkAccessToElement($id, $isPid = false)
	{
		...
		if (!$isPid) {
			$content = $this->Database
				->prepare("SELECT * FROM orm_avisota_newsletter_content WHERE id=?")
				->execute($id);
			if ($content->next()) {
				$id = $content->pid;
			}
				...
				$this->log(
					...
					'orm_avisota_newsletter_content checkAccessToElement()',
					TL_ERROR
				);
				return false;
			}
...
<table>' . $this->Content->getNewsletterElement($contentData['id']) . '</table>
	...
	public function toggleVisibility($contentId, $visible)
	{
		...
		$this->createInitialVersion('orm_avisota_newsletter_content', $contentId);

		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['orm_avisota_newsletter_content']['fields']['invisible']['save_callback'])) {
			foreach (
				$GLOBALS['TL_DCA']['orm_avisota_newsletter_content']['fields']['invisible']['save_callback'] as
				$callback
			) {
		...
		$this->Database
			->prepare(
			"UPDATE orm_avisota_newsletter_content SET tstamp=" . time() . ", invisible='" . ($visible ? ''
				: 1) . "' WHERE id=?"
		)
			->execute($contentId);

		$this->createNewVersion('orm_avisota_newsletter_content', $contentId);
	}
	...
	public function dcaGetNewsletterAreas($dc)
	{
		$category = $this->Database
			->prepare(
			"SELECT c.* FROM orm_avisota_newsletter_category c INNER JOIN orm_avisota_newsletter n ON c.id=n.pid INNER JOIN orm_avisota_newsletter_content nle ON n.id=nle.pid WHERE nle.id=?"
		)
	...
	{
		if ($name == 'orm_avisota_newsletter_content') {
			if ($this->Input->get('table') == 'orm_avisota_newsletter_content' && $this->Input->get('act') == 'edit') {
