...
class AvisotaBackendStylepickerDCA extends tl_stylepicker4ward
{
	...
	public function saveAvisotaNewsletterCEs($val, $dc)
	{
		...
		$this->truncateTargets($dc->id, 'orm_avisota_mailing_content');
		...
		if (is_array($vals)) {
			...
			foreach ($secs as $sec) {
				foreach ($vals as $val) {
					$this->saveTarget($dc->id, 'orm_avisota_mailing_content', 'cssID', $sec, $val);
				}
			}
		}
		return null;
	...
	public function loadAvisotaNewsletterCEs($val, $dc)
	{
		...
		$targets = $this->Database
			...
			->execute($dc->id, 'orm_avisota_mailing_content');
		while ($targets->next()) {
			$return[] = $targets->cond;
		}
	...
	public function getAvisotaNewsletterSections()
	{
		...
		$category = $this->Database
			->query('SELECT * FROM orm_avisota_mailing_category WHERE areas!=\'\'');
		while ($category->next()) {
			$ret = array_merge(
				$ret,
