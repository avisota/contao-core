...
class NewsletterTheme extends \Backend
{
	...
	public function checkPermission()
	{
		...
		$GLOBALS['TL_DCA']['orm_avisota_mailing_theme']['list']['sorting']['root'] = $root;

		// Check permissions to add channels
		if (!$this->User->hasAccess('create', 'avisota_newsletter_category_permissions')) {
			$GLOBALS['TL_DCA']['orm_avisota_mailing_theme']['config']['closed'] = true;
		}
		...
		switch ($this->Input->get('act')) {
			...
			case 'edit':
				...
				if (!in_array($this->Input->get('id'), $root)) {
					...
					if (is_array($newRecord['orm_avisota_mailing_theme']) && in_array(
						$this->Input->get('id'),
						$newRecord['orm_avisota_mailing_theme']
					)
					) {
			...
			case 'show':
				if (!in_array($this->Input->get('id'), $root) || ($this->Input->get(
					...
					$this->log(
						...
						'orm_avisota_mailing_theme checkPermission',
						TL_ERROR
					);
	...
	{
		return ($this->User->isAdmin || count(preg_grep('/^orm_avisota_mailing_theme::/', $this->User->alexf)) > 0)
	...
	public function generateAlias($value, $dc)
	{
		...
		$aliasResultSet = $this->Database
			->prepare("SELECT id FROM orm_avisota_mailing_theme WHERE alias=?")
			->execute($value);
