	...
	public function compile()
	{
		...
		if ($arrSession['newsletter'])
		{
			...
			if ($objNewsletter->next())
			{
				...
				if ($arrSession['recipient'])
				{
					$objLink = $this->Database->prepare("SELECT url,hits,times FROM tl_avisota_newsletter_link_hit WHERE pid=? AND recipient=? ORDER BY hits DESC")->execute($arrSession['newsletter'], $arrSession['recipient']);
				}
				else
				{
					$objLink = $this->Database->prepare("SELECT url,hits,times FROM (SELECT url,SUM(hits) as hits,times FROM tl_avisota_newsletter_link_hit WHERE pid=? GROUP BY url) t ORDER BY hits DESC")->execute($arrSession['newsletter']);
				}
				
				$arrLinkHits = array();
				$arrLinkTimeline = array();
				$objLink->reset();
				while ($objLink->next())
				{
					$arrLinkHits[] = array('url'=>$objLink->url, 'hits'=>$objLink->hits);
					$arrTimes = deserialize($objLink->times, true);
					foreach ($arrTimes as $strTime)
					{
						$arrLinkTimeline[$strTime]++;
					}
				}
				$this->Template->links = $arrLinkHits;
				$this->Template->link_timeline = $arrLinkTimeline;
				...
				if ($arrSession['recipient'])
				{
					...
					$this->Template->reads  = $this->Database->prepare("SELECT COUNT(pid) as total FROM tl_avisota_newsletter_read WHERE recipient=? AND readed=?")->execute($arrSession['recipient'], 1)->total;
					// total number of newsletters the recipients reacts on (clicked a link)
					$this->Template->reacts = $this->Database->prepare("SELECT COUNT(pid) as total FROM tl_avisota_newsletter_link_hit WHERE recipient=? AND hits>0 GROUP BY pid")->execute($arrSession['recipient'])->total;
				}
				else
				{
					...
					$this->Template->reads  = $this->Database->prepare("SELECT COUNT(recipient) as total FROM tl_avisota_newsletter_read WHERE pid=? AND readed=?")->execute($objNewsletter->id, 1)->total;
					// total number ov recipients taht reacts on this newsletter (clicked a link)
					$this->Template->reacts = $this->Database->prepare("SELECT COUNT(recipient) as total FROM tl_avisota_newsletter_link_hit WHERE pid=? AND hits>0 GROUP BY recipient")->execute($objNewsletter->id)->total;
					// day+hour based reads
					$this->Template->timeline = $this->Database->prepare("SELECT time,COUNT(time) as total FROM (SELECT DATE_FORMAT(FROM_UNIXTIME(tstamp), '%%d.%%m.%%Y %%H') as time FROM tl_avisota_newsletter_read WHERE pid=? AND readed='1') t GROUP BY time ORDER BY time")->execute($objNewsletter->id)->fetchAllAssoc();
				}
			}
			else
			{
