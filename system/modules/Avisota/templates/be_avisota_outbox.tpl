...
<table class="prev_header outbox" summary="" cellpadding="0" cellspacing="0" width="100%">
  ...
  <tr class="row_<?php echo $k ?><?php if ($outbox['id'] == $this->Input->get('highlight')): ?> row_highlight<?php endif ?>">
    ...
    <td class="col_5"><a href="<?php echo $GLOBALS['TL_CONFIG']['websitePath'] ?>/contao/main.php?do=avisota_outbox&amp;act=details&amp;id=<?php echo $outbox['id'] ?>" title="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['details'][1]) ?>"><img src="system/modules/Avisota/html/outbox_details.png" alt="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['details'][0]) ?>" /></a>&nbsp;<a href="<?php echo $GLOBALS['TL_CONFIG']['websitePath'] ?>/contao/main.php?do=avisota_outbox&amp;act=remove&amp;id=<?php echo $outbox['id'] ?>&amp;token=<?php echo $outbox['token'] ?>" title="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['remove'][1]) ?>" onclick="return confirm('<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['remove'][2]) ?>');"><img src="system/modules/Avisota/html/outbox_remove.png" alt="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['remove'][0]) ?>" /></a><?php if ($this->beSend && $outbox['outstanding'] > 0): ?>&nbsp;<a href="<?php echo $GLOBALS['TL_CONFIG']['websitePath'] ?>/contao/main.php?do=avisota_outbox&amp;id=<?php echo $outbox['id'] ?>&amp;token=<?php echo $outbox['token'] ?>" title="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['send'][1]) ?>"><img src="system/modules/Avisota/html/send_go.png" alt="<?php echo specialchars($GLOBALS['TL_LANG']['tl_avisota_newsletter_outbox']['send'][0]) ?>" /></a><?php endif ?></td>
  </tr>
  <?php endforeach; ?>
  </tbody>
</table>
