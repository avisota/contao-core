...
class ModuleAvisotaList extends Module
{
	...
	public function compile()
	{
		...
		$objNewsletter = $this->Database
			->prepare("SELECT * FROM tl_avisota_newsletter WHERE sendOn > 0 AND pid IN (" . implode(',', $this->avisota_categories) . ") ORDER BY sendOn DESC");
		if ($intLimit > 0)
		{
		...
		while ($objNewsletter->next())
		{
			...
			if ($objViewPage) {
				...
				if ($arrViewOnlineCache[$objNewsletter->pid]) {
					$arrNewsletter['href'] = $this->generateFrontendUrl($arrViewOnlineCache[$objNewsletter->pid]->row(), $strParams);
				} else {
					$arrNewsletter['href'] = '';
				}
			}
		...
		$objNewsletter = $this->Database
			->prepare("SELECT COUNT(id) as `count` FROM tl_avisota_newsletter WHERE sendOn > 0 AND pid IN (" . implode(',', $this->avisota_categories) . ")")
			->execute();
